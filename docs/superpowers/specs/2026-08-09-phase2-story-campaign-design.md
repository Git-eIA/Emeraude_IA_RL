...
- New `DESTINATIONS` entries for the lab interior cell (where the Birch cutscene
  auto-runs) and the Route 101 shoes-trigger cell. Exact cells frozen by the probe.
...
- `has_item(item_id, min_qty=1)` → the hard part. Emerald encrypts item
  ...
  at `gSaveBlock2Ptr + 0xAC`. Bag pockets live in SaveBlock1. `has_item` reads
  the Poké Balls pocket, finds `item_id` (Poké Ball = `0x4`), decrypts the
  quantity, compares to `min_qty`. All offsets (pocket base, slot size,
  securityKey offset) frozen live by the probe before wiring.
...
- New `PHASE2_CAMPAIGN` tuple: return Littleroot → enter lab → Pokédex → Poké
  Balls → running shoes (+ Flora talks only if the probe finds them). Each story
  milestone carries a `story_target` predicate (e.g. `lambda r: r.has_pokedex()`).
- `run_campaign` emits `Order(dest, "story", "win")` for story milestones and
  ...
  non-`"arrived"` / non-`"won"` today.

## Data flow

```
run_campaign(PHASE2_CAMPAIGN)
  └─ per Milestone with story_target:
...
map transitions; `travel_to` routes via `memory`'s portals — the same ones the
northbound rival leg already crossed. The Phase 2 run therefore starts from a
`memory` that knows those portals (survey first, or a pre-populated memory) — the
same assumption `run_campaign` already makes for the rival leg.
...
- trigger cells (lab Birch cutscene, Route 101 Mom),
...
- **Lab script-gate order**: you must talk to Birch (Pokédex) before you can
  leave; the exact cell where the cutscene auto-runs is frozen by the probe.
...
- **Shoes trigger**: Mom auto-runs on a specific southbound Route 101 tile → cell
  frozen by the probe.
- **Multi-map return nav** depends on a `memory` that knows the return-path
  portals (same assumption as the rival leg).
...
- `env/campaign.py` — `Milestone.story_target`, `PHASE2_CAMPAIGN`, `run_campaign`
  story dispatch.
