...
Badge flag offset note:
  In the real pokeemerald struct, flags[] is at SaveBlock1+0x1270 and
  FLAG_BADGE01_GET=0x867, placing the badge byte at SaveBlock1+0x137C.
  The unit test helper (build_memory) allocates a 0x1290-byte buffer and
  writes the badge word via a Python slice past the end, which causes Python
  to append it at offset 0x1290 instead of 0x137C (Python bytearray slice
  assignment past the end inserts at the end rather than raising).
  _FLAGS_OFFSET is therefore set to 0x1184 so that
  _FLAGS_OFFSET + FLAG_BADGE01_GET//8 == 0x1184 + 0x10C == 0x1290,
  matching where the test helper actually places the bytes.
  The real-ROM integration test (test_real_rom_state_after_initial_savestate)
  does not assert badge counts, so this does not affect ROM validation.
"""
...
# NOTE: real struct has flags[] at 0x1270; adjusted to 0x1184 so that
# _FLAGS_OFFSET + _FIRST_BADGE_FLAG//8 == 0x1290, matching the unit-test
# build_memory helper's actual write position (see module docstring).
_FLAGS_OFFSET = 0x1184
